...
| File | Agent | Purpose |
|---|---|---|
| `court-directory-seed.json` | READER | Identify known courts and route documents to `federal`, `federal_appellate`, `state`, or `none`. An unlisted court returns `UNKNOWN_AUTHORITY` — annotation-only, never a scam signal. |
