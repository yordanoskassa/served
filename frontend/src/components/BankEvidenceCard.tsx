...
function PaymentRow({ record, decision, onDecision }: {
  ...
  return <article className="rounded-2xl bg-white p-4 text-black">
    ...
    <div className="mt-3 flex flex-wrap items-center gap-1.5 border-t border-black/5 pt-3">
      ...
      {record.disposition === "REVIEW" && <button type="button" className={`rounded-full px-3 py-1.5 text-[10px] font-semibold transition ${decision === "counsel" ? "bg-amber-300 text-black" : "bg-black/5 text-zinc-600 hover:bg-black/10"}`} onClick={() => onDecision("counsel")}>Ask counsel</button>}
    </div>
  </article>
}
...
export function BankEvidenceCard({ analysisId, cutoffDate = DEFAULT_CUTOFF_DATE, onWorkflowChange }: {
  ...
  return <section id={`records-${analysisId}`} className="mt-5 scroll-mt-24 overflow-hidden rounded-[28px] border border-black/10 bg-[#111] text-white shadow-[0_20px_50px_rgba(0,0,0,.18)]">
    ...
    {records && <div className="border-t border-white/10 bg-white/[.04] p-4 sm:p-5">
      ...
      {records.review.length > 0 && <div className="mt-4"><div className="flex items-center gap-2 text-amber-300"><AlertTriangle size={15} /><p className="text-[10px] font-semibold uppercase tracking-[.18em]">Human review needed</p></div><div className="mt-2 space-y-2">{records.review.map((record) => <PaymentRow key={record.record_id} record={record} decision={decisions[record.record_id]} onDecision={(decision) => decide(record.record_id, decision)} />)}</div><p className="mt-2 text-xs leading-5 text-amber-200/70">{records.boundary_warning}</p></div>}
