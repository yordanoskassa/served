  ...
  return <div className="bg-bg-base px-8 pb-24 md:px-16 lg:px-20">
    <section id="workflow" className="mx-auto max-w-7xl border-t border-black/10 py-20">
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {workflow.map(({ icon: Icon, title, text }, index) => <article className="rounded-[14px] border border-black/10 bg-card/70 p-6 shadow-[0_1px_0_rgba(255,255,255,.7)_inset]" key={title}><div className="flex items-center justify-between border-b border-black/[.08] pb-4"><span className="grid size-10 place-items-center rounded-full bg-brand-soft"><Icon size={17} /></span><span className="font-display text-xs text-zinc-500">§ 0{index + 1}</span></div><h3 className="mt-6 font-display text-2xl font-semibold tracking-[-.02em]">{title}</h3><p className="mt-3 text-sm leading-6 text-zinc-500">{text}</p></article>)}
      </div>
    </section>
    ...
    <section id="bank-connection" className="mx-auto max-w-7xl border-t border-black/10 py-20">
      ...
      <div className="mt-10 grid gap-2 sm:grid-cols-3" aria-label="Plaid Sandbox connection preview">
        {[{ icon: Landmark, title: "First Platypus Bank", note: "Fictional Sandbox institution" }, { icon: DatabaseZap, title: "Seeded business checking", note: "28 synthetic transactions" }, { icon: ShieldCheck, title: "No real financial data", note: "Safe, repeatable judge demo" }].map(({ icon: Icon, title, note }) => <div className="flex min-h-20 items-center gap-3 rounded-[10px] border border-black/10 bg-card/80 px-4 shadow-[0_6px_18px_rgba(31,45,56,.04)]" key={title}><span className="grid size-9 shrink-0 place-items-center rounded-full bg-brand-soft"><Icon size={16} /></span><div><p className="text-xs font-semibold">{title}</p><p className="mt-1 text-[10px] text-zinc-500">{note}</p></div></div>)}
      </div>
      <p className="mt-3 text-center text-[10px] text-zinc-400">The hackathon demo uses Plaid Sandbox and a fictional institution.</p>

      <div className="mt-8 overflow-hidden rounded-[16px] border border-white/10 bg-[#172635] p-6 text-white shadow-[0_22px_50px_rgba(31,45,56,.16)] sm:p-8">
    ...
    <section id="record-matching" className="mx-auto grid max-w-7xl gap-8 border-t border-black/10 py-20 lg:grid-cols-[1.05fr_.95fr] lg:items-center">
      ...
      <div className="overflow-hidden rounded-[16px] border border-white/10 bg-[#172635] p-6 text-white shadow-xl">
    ...
    <section id="privacy" className="mx-auto max-w-7xl rounded-[16px] border border-black/10 bg-card/70 p-8 shadow-[0_1px_0_rgba(255,255,255,.7)_inset] sm:p-12">
