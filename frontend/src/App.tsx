...
export function App() {
  ...
  return <GoogleOAuthProvider clientId={clientId}><Dialog open={showAuth} onOpenChange={setShowAuth}>{landing}<DialogContent className="max-w-md border-0 bg-transparent p-0 shadow-none"><DialogTitle className="sr-only">Sign in to Served</DialogTitle><DialogDescription className="sr-only">Sign in with Google to open {entryLabel(entryIntent)} in your private Served workspace.</DialogDescription><LoginPage destination={entryLabel(entryIntent)} /></DialogContent></Dialog></GoogleOAuthProvider>
}
