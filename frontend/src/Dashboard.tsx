    ...
    <Tabs value={activeTab} onValueChange={setActiveTab} className="min-h-screen bg-bg-base text-[#1a1a1a] selection:bg-brand-green selection:text-black">
      ...
      <main className="lg:ml-64">
        ...
        <TabsList className="mx-5 mt-4 grid h-auto grid-cols-4 rounded-full bg-black/5 p-1 lg:hidden">
          ...
          <TabsTrigger value="agents" className="rounded-full px-2 py-2 text-[11px] data-[state=active]:bg-white">Agents</TabsTrigger>
        ...
        <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:py-12">
          ...
          <TabsContent value="agents" className="mt-0"><section className="rounded-[28px] bg-[#1a1a1a] p-6 text-white shadow-[0_20px_60px_rgba(0,0,0,.12)]">
            ...
            <TooltipProvider><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              {agentState === "ready" && agents.map((agent, index) => <article className="rounded-2xl border border-white/10 bg-white/[.06] p-4" key={agent.name}><div className="flex items-center justify-between"><span className="grid size-7 place-items-center rounded-full bg-brand-soft text-xs font-semibold text-black">{index + 1}</span><Tooltip><TooltipTrigger asChild><span tabIndex={0} className={`size-2 rounded-full ${agent.enabled && !agent.last_error ? "bg-brand-soft" : "bg-orange-400"}`} /></TooltipTrigger><TooltipContent className="bg-white text-black">{agent.enabled && !agent.last_error ? "Ready" : "Unavailable"}</TooltipContent></Tooltip></div><p className="mt-4 text-sm font-medium capitalize">{agent.name.replaceAll("_", " ")}</p><p className="mt-2 text-xs leading-5 text-white/45">{agent.description}</p></article>)}
            </div></TooltipProvider>
          </section></TabsContent>
